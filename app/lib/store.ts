...
interface WalletState {
  ...
  // 添加新方法用于检查支付状态
  checkPaymentStatus: (address: string) => Promise<void>
}
    ...
    (set, get) => ({
      ...
      setWalletState: (connected: boolean, address: string | null) => {
        ...
        if (connected && address) {
          get().checkPaymentStatus(address);
        }
      },
      ...
      // 检查该地址是否已支付
    checkPaymentStatus: async (address: string) => {
      console.log('Store: Checking payment status for address:', address);
      try {
        const response = await fetch(`/api/check-payment-status?address=${address}`);
        console.log('Store: Payment status response:', response.status);
        const data = await response.json();
        console.log('Store: Payment status data:', data);
        set({ hasUnlimitedAccess: data.hasPaid });
      } catch (error) {
        console.error('Store: Failed to check payment status:', error);
      }
    }
